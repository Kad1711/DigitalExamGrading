...
import StudentExcelImportModal from "../components/classes/StudentExcelImportModal";
import { AddStudentModal, EditStudentModal } from "../components/classes/StudentModals";
import {
  Users,
  GraduationCap,
  Plus,
...
export default function TeacherClassesPage() {
  ...
  const canManageCurrentClassStudents = useMemo(() => {
    return canManageAllClasses || (isTeacher && isClassAssignedToMe(selectedClass));
  }, [canManageAllClasses, isTeacher, isClassAssignedToMe, selectedClass]);
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
              ...
              <div className="bg-white rounded-xl border border-slate-200 shadow-xs p-6 space-y-6">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 pb-4 border-b border-slate-100">
                  ...
                  <div className="flex items-center flex-wrap gap-2">
                    {canManageCurrentClassStudents && students.length > 0 && (
                      <>
                        <Button
                          variant="outline"
                          size="sm"
                ...
                {isTeacher && isClassAssignedToMe(selectedClass) && (
                  <div className="flex items-start gap-3 p-3 rounded-xl bg-blue-50/80 border border-blue-200 text-blue-900 text-xs animate-in fade-in duration-150">
                    ...
                    <div>
                      <span className="font-bold text-blue-950">Lớp phân công giảng dạy:</span> Bạn là giáo viên phụ trách lớp <strong className="font-semibold text-blue-950">{selectedClass.name}</strong>. Bạn có quyền thêm, sửa, xóa học sinh, chuẩn hóa SBD và import danh sách từ Excel cho lớp học này.
                    </div>
                  </div>
                )}

                {/* Bulk Student Actions Banner */}
                {canManageCurrentClassStudents && selectedStudentIds.length > 0 && (
      ...
      <AddStudentModal
        ...
        getSuggestedSbd={getSuggestedSbd}
      />
      ...
      <EditStudentModal
        ...
        editStudentCode={editStudentCode}
        setEditStudentCode={setEditStudentCode}
        editStudentFullName={editStudentFullName}
        setEditStudentFullName={setEditStudentFullName}
        editStudentDob={editStudentDob}
        setEditStudentDob={setEditStudentDob}
        ...
        editStudentError={editStudentError}
        setEditStudentError={setEditStudentError}
      ...
      <StudentExcelImportModal
        ...
        importingStudents={importingStudents}
        analyzingFile={analyzingFile}
        sbdMode={sbdMode}
        setSbdMode={setSbdMode}
        mappingStudentCodeCol={mappingStudentCodeCol}
        setMappingStudentCodeCol={setMappingStudentCodeCol}
        isSelectedCodeSerial={isSelectedCodeSerial}
        mappingDobCol={mappingDobCol}
        setMappingDobCol={setMappingDobCol}
        nameMode={nameMode}
        setNameMode={setNameMode}
        mappingFullNameCol={mappingFullNameCol}
        setMappingFullNameCol={setMappingFullNameCol}
        mappingLastNameCol={mappingLastNameCol}
        setMappingLastNameCol={setMappingLastNameCol}
        mappingFirstNameCol={mappingFirstNameCol}
        setMappingFirstNameCol={setMappingFirstNameCol}
